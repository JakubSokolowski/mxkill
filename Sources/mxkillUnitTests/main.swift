...
func testDestructiveCursorHotSpotPlacesIconUpLeftOfPointer() {
    ...
    expectEqual(DestructiveCursor.hotSpot(for: DestructiveCursor.defaultSize), CGPoint(x: 30, y: 14), "destructive cursor hot spot")
}
...
func testCursorOverlayFrameUsesHotSpot() {
    let frame = CursorOverlay.frame(
        ...
        hotSpot: CGPoint(x: 30, y: 14)
    )

    expectEqual(frame, CGRect(x: 70, y: 186, width: 44, height: 44), "cursor overlay frame")
}
